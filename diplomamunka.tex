...
\newcommand{\gerrit}[1]{%
    \textcolor[HTML]{44A51C}{\origurl{https://codereview.qt-project.org/\#/c/#1}}
}
...
\begin{itemize}
    \item \gerrit{81252}
    \item Qt 5.4
    \item Qt 5.5: API átalakítva, experimental-ból átmozgatva
    \item Hiányzó Quick API megvalósítása, widget már kész, mintaként szolgált
    \item Experimental API: csak Quick, próbáljuk eltüntetni
    \item Az API már megváltozott, ez az implementáció a QtWebKit API-t másolta
    \item Mi az a QML ListModel?
    \item URL és Title van tárolva
    \item \texttt{plugin.cpp}
\end{itemize}
...
\begin{table}[h!]
    ...
    \caption{
        ...
        Az új \texttt{WebEngineHistory} QML típus
    }
\end{table}
...
\begin{table}[ht!]
    \centering
    \begin{tabular}{ | l | l | p{152pt} | }
        \hline
        \textbf{Property} & \textbf{Típus} & \textbf{Leírás} \\ \hline

        \texttt{navigationHistory} & \texttt{WebEngineHistory} &
        Az aktuális view-hoz (tab-hoz) tartozó \texttt{WebEngineHistory} példány
        \\ \hline

        \makecell[l]{\texttt{currentNavigation} \\ \texttt{EntryIndex}} & \texttt{int} &
        \makecell[l]{Az aktuális oldal indexe a \\ history-ban}
        \\ \hline

        \texttt{goBackTo(int)} & \texttt{slot} &
        A paraméterben átadott index-el lép vissza a \texttt{backItems} listában és tölti
        be az oldalt
        \\ \hline

        \texttt{goForwardTo(int)} & \texttt{slot} &
        A paraméterben átadott index-el lép előre a \texttt{forwardItems} listában és tölti
        be az oldalt
        \\ \hline
    \end{tabular}
    \caption{
