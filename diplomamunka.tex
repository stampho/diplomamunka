...
A \texttt{Qt} első verziója 1995-ben adta ki a Trolltech. Akkor még a projekt egy
...
platformja a \texttt{Windows Phone} lett, így a Nokia elkezdte fokozatosan leépíteni a
szoftver kutatás-fejlesztés részlegét. 2012-re a \texttt{Qt} teljes egészében a Digia
tulajdonába került.

A 2012-es év más változást is hozott, az év végén megjelent a \texttt{Qt} 5.0-ás verziója.
A verzió egyik nagy újítása a moduláris felépítés volt. A \texttt{C++} API-k modulokba
kerültek, a modulok pedig saját git repository-t is kaptak. Létrehoztak egy \texttt{qt5}
``top-level'' repository-t is, amely hivatkozásokat (git submodule) tartalmaz a modulok
repository-jaira, definiálja a függőségeket köztük, tartalmazza build-hez szükséges
script-eket (pl. configure) és specifikációkat (mkspecs) és más script-eket is.

A verzió másik újítása \texttt{QtQuick2} volt \dots
