...
eltávolítottam a \texttt{QNetworkAccessManager} használatát és az autentikácóhoz szükséges
workaround-ot is.

Az bővített API-val most már a WebEngine Widgets modul támogatja az olyan ikonok használatát
is, amelyek több különböző méretű ikont ágyaznak magukba. Ezekhez, és az új \texttt{icon}
property-hez teszteket is készítettem, a \texttt{QWebEnginePage} és \texttt{QWebEngineView}
API-k dokumentációit bővítettem és naprakésszé tettem.
...
\begin{itemize}
    \item Quick API-hoz icon image provider: QQuickWebEngineFaviconProvider
...
\begin{itemize}
    \item Az oldalhoz tartozó ikonok elérése egy ikonban
    \item Átméretezéskor, a mérethez legjobban illeszkedő ikon betöltése
        \begin{itemize}
            \item Widget: QIcon API-n keresztül
            \item Quick: QQuickWebEngineFaviconProvider-en keresztül
        \end{itemize}
    \item Quick tesztelés ugyanúgy mint a multi-sized ikon teszt esetében
    \item Tesztelés során felfedezett bug javítása a Qt Declarative komponensben
\end{itemize}
...
\begin{itemize}
    \item A history bejegyzésekhez tartozó ikonok támogatása Quick-ben
\end{itemize}
