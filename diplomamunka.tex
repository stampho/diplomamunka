...
\begin{itemize}
    \item Multi-processz architektúra megvalósítása
    \item Content module csak rendereléshez szükséges kódot tartalmazza
    \item HTML5 és hardveres GPU gyorsítás támogatása
    \item Cél: a core kód elválasztás minden mástól
    \item Content API: a hiányzó feature-ök megvalósítására
        \begin{itemize}
            \item Tervezési minták: Delegate \& Observer interfaces
            \item Callback függvények
            \item Controller
        \end{itemize}
    \item az interface-ket és a callback függvényeket a beágyazó alkalmazás implementálja
    \item beágyazó alkalmazás: pl. chrome
    \item A Content modul hívja a callback-eket
    \item WebKit API-ra hasonlít a könnyebb átállás miatt
    \item Unstable API: Mit is jelent ez?
\end{itemize}

\subsection{Felépítés}
\texttt{Chromium} 3 réteg:
\begin{itemize}
    \item Chrome: UI és feature-ök/browser komponensek (history, favicon)
    \item Blink: rendering és V8 JavaScript engine közös processzben (single process)
    \item Content Module: HTML5 és hardveres GPU gyorsítás
\end{itemize}
Hogyan is áll össze ez az egész \dots
...
\begin{figure}[h]
    ...
    \caption{
        ...
        (forrás: \url{https://www.chromium.org} \cite{bib-chromium-content})
    }
\end{figure}
\dots
