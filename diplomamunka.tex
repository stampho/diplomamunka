...
\begin{itemize}
    ...
    \item Message Bubble
    \item HTML5 feature
    \item Chromium callback-ek megvalósítása
        \begin{itemize}
            \item \texttt{WebContentsDelegate::ShowValidationMessage}
            \item \texttt{WebContentsDelegate::HideValidationMessage}
            \item \texttt{WebContentsDelegate::MoveValidationMessage}
        \end{itemize}
    \item Quick és Widget API-ban kétféleképpen megrajzolt buborék
    \item Test Support API teszt
    \item Qt 5.5
    \item QML Canvas Item-re rajzol, nincs ikon
    \item \texttt{UIDelegatesManager}
    \item \texttt{QT\_UI\_DELEGATES} makró
    \item \texttt{QtWebEngineWidgetUI} namespace
    \item Widget: \texttt{QPainter}-el rajzol -> új widget
\end{itemize}
\pagebreak
