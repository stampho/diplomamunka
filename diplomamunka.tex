...
\end{verbatim}
A böngésző alkalmazás ezt az ikont tetszőleges célokra használhatja fel, jellemzően az URL
bar-on vagy a tab-on jelenítik meg.
Az oldalhoz rendelt ikon lehet \textit{touch icon} is. A touch icon jellemzően nagyobb
felbontású, mint a favicon és főleg érintőképernyős készülékeken használják az előzmények
és/vagy könyvjelzők megjelenítésére. Egy oldalhoz több favicon és/vagy touch icon is
tartozhat, jellemzően a böngésző alkalmazás választ közülük.

A \textbf{Favicon Manager} ezeket az ikonokat tölti le a Chromium Content API-n keresztül,
és nyújt lehetőséget az elérésükre. Megvalósítását több cél is motiválta:
\begin{itemize}
    \item Az jelszóval védett oldalak betöltése esetén ne legyen szükség workaround-ra a
        böngésző alkalmazás kódjában ahhoz, hogy le lehessen tölteni a favicon-t
...
Mivel a Favicon Manager megvalósítása sok részfeladatot foglal magába, ezért funkcióhoz
tartozó módosításokhoz issue is készült:
\begin{center}
    \qtbug{51179}
\end{center}
Az alább bemutatott patch-ek a részfeladatokat valósítják meg.
...
jelenti. Ez az osztály fogja letölteni a megjelenített oldalhoz tartozó ikonokat, fogja
kiválasztani közülük a megfelelőt, jelez az API-nak ha az ikon megváltozott, teszi azt
elérhetővé és nyújt belső API-t az ikonok manipulálására egy jövőbeni publikus API számára.
A megvalósításban már a C++11 szabvány \textit{range-based for} nyelvi eleme van felhasználva
az optimálisabb kód érdekében.

Az ikonok letöltéséhez a Content API \texttt{WebContents::DownloadImage()} \\
függvényét használja, kivéve, ha az ikon a QRC-ben (Qt Resource System) van tárolva,
ugyanis ebben az esetben nincs szükség letöltésre.
Alapvetően a Favicon Manager az összes web oldalhoz tartozó ikont megpróbálja letölteni és
a letöltötteket egy \texttt{QMap}-ben tárolja, ahol a kulcs az ikon URL-je. Mielőtt a
manager letöltené az ikont, ellenőrzi, hogy le van e már töltve a map-ben, így nem kell az
ikonokat minden oldal betöltés után újra és újra a Chromium-tól elkérni.

Minden tab-nak (WebEngineView-nak) saját Favicon Manager-re van, ebből kifolyólag
előfordulhat egy ikon többször is tárolva van a memóriában. Ennek a modellnek az az előnye,
hogy gyorsabb lehet a szükséges ikon megkeresése (kevesebb bejegyzés) és az implementáció
is jelentősen egyszerűbb. A WebEngineView API a \texttt{iconChanged()} signal-al jelzi,
hogy ha megváltozott a view-hoz tartozó ikon. Ez az ikon az API-n keresztül lekérhető,
az API pedig a Favicon Manager-től kéri el. A Favicon Manager a letöltött ikonok közül
a legnagyobb felbontásút fogja felajánlani.
...
az első Favicon Manager-t érintő patch jelentős részét teszi ki.
...
A Favicon Manager megvalósítása során az is javítva lett, hogy a Widget és a Quick API-ban
az \texttt{iconChanged()} signal ne legyen kétszer kiküldve, olyan oldal betöltése esetén,
amikor az oldalhoz tartozó ikon nem változott.
...
A Favicon Manager Core modul-beli megvalósításával együtt review-ra lettek bocsájtva publikus
Favicon Manager API megvalósítások Widget-hez és QML-hez is. Mérlegelés után arra döntésre
jutottunk, hogy a publikus API nem kerül be az 5.7-es verzióba. Mivel a Core modulban
lévő Favicon Manager megvalósítás tartalmaz olyan kódot és funkciókat, amelyek a publikus
API-t készítették elő, ezért azok eltávolításra kerültek. Az alábbi javasolt API
funkcionalitások az 5.8-as verzióban kerülnek újra felülvizsgálatra:
\begin{itemize}
    \item A látható oldalhoz tartozó ikonok listája lekérése
    \item Ikon explicit letöltése és nyilvántartása a Favicon Manager-ben
    \item A Favicon Manager cache ürítése
\end{itemize}
Az alábbi funkciók pedig végleg eltávolításra kerültek:
\begin{itemize}
    \item Ikon törlése a Favicon Manager cache-ből URL alapján (az ikon újra letöltésének
        kikényszerítéséhez szükséges)
    \item Opció régi API működésére: a legjobb minőségű ikon helyett a Favicon Manager
        az első favicon-t tölti le
\end{itemize}

TODO: A kiválasztott ikon URL-jének tárolása a \texttt{NavigationEntry}-ben \dots
...
\begin{itemize}
    \item bestFavicon teszt workaround
    \item Probléma: 49-es Chromium óta az \texttt{iconChanged} signal majdnem mindig a \\
        \texttt{loadFinished} után érkezik.
    \item Két eset lehetséges:
        \begin{itemize}
            \item 1 \texttt{iconChanged} signal érkezik: az \texttt{iconChanged} a
                \texttt{loadFinished} előtt küldődik
            \item 2 \texttt{iconChanged} signal érkezik: a \texttt{loadFinished} reseteli az
                ikont, majd a második \texttt{iconChanged} tartalmazza az oldal ikonját
        \end{itemize}
    \item A második \texttt{iconChanged} signal küldése sokszor ``késik''
    \item Ha késik az elrontja a következő teszt eredményét is
    \item Workaround: ha az \texttt{iconChanged} signal-ban kapott URL üres, akkor vár
        a második signal-ra
\end{itemize}
...
\begin{itemize}
    ...
    \item Letilthatóak lehessenek az ikonok (sávszélesség, sebesség, saját icon manager)
    \item Desktop-on alapból nincsenek bekapcsolva a touch ikonok, de engedélyezni
        lehessen őket
    \item 2 beállítás:
        \begin{itemize}
            \item \texttt{AutoLoadIconsForPage}: automatikusan töltse le az ikont a \\
                Qt WebEngine
            \item \texttt{TouchIconsEnabled}: töltse le a touch ikonokat is
        \end{itemize}
    \end{itemize}
...
\begin{itemize}
    \item Widget API ``feljavítása''
        \begin{itemize}
            \item \texttt{icon} property a \texttt{QIcon}-hoz,
                ha változik \texttt{iconChanged}
            \item \texttt{iconUrl} property a \texttt{QUrl}-hez,
                ha változik \texttt{iconUrlChanged}
        \end{itemize}
    \item Nem csak az URL-t lehet lekérdezni, hanem magát az ikont is
    \item A Widget demobrowser-ből el lett távolítva a workaround
    \item Több ikont tartalmazó ikon (``multi-sized icon'') teszt
    \item Dokumentáció frissítése
\end{itemize}
...
\begin{itemize}
    \item Még nem végleges, valószínűleg 2 patch-be lesz szétszedve
    \item Quick API-hoz icon image provider
    \item problémák: hogyan kössük be a manager-t? Hol kössük be a provider-t az engine-be?
    \item tesztelés:
        \begin{itemize}
            \item Multi sized icon: minden ikon szürke színű és az árnyalat kódolja az
                ikon méretét (pl.: 32x32 -> rgb(32, 32, 32)
            \item Egy pixel kiolvasásával ellenőrizhető, hogy a favicon provider
                a megfelelő méretű ikont töltötte e be
            \item hogyan vegyünk ki egy QML item-ből egy pixelt?
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item 5.7-ben jelent meg
    \item 5.8-ban bővül
        \begin{itemize}
            \item Favicon Manager API
            \item Favicon Database: \\
                \qtbug{51184}
        \end{itemize}
\end{itemize}
\pagebreak
