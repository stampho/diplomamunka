...
\begin{itemize}
    ...
    \item 4 patch:
        \begin{itemize}
            \item WIDGET: bug fix -> ha nincs megadva felhasználónév és jelszó
                (empty credentials) az nem cancel
            \item CORE: \texttt{AuthenticationDialogController} megvalósítása, \\
                    widget API igazítása az új internal Core API-hoz
                \item WIDGET: bug fix (widget demobrowser) -> jelszóval védett oldalak esetén
                    nem tudja betölteni a favicon-t
                \item QUICK: UI Delegate megvalósítása, saját authentication dialog,
                    a Qt Quick Dialogs nem tartalmaz erre megfelelő dialog ablakot
        \end{itemize}
