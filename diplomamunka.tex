...
a \texttt{-{}-lang} kapcsolóval. Erre szükség is van, elsősorban tesztelésnél, ahol a
hiba üzeneteket, hiba eseteket is ellenőrizni kell.

A kapcsoló szükségessége éppen a Form Validation funkció tesztelésénél merült fel,
amiről hiba jelentés (bug report) is készült:
\begin{center}
    \qtbug{45715}
\end{center}
A tesztelésnél angol nyelvű hiba üzeneteket szeretnénk ellenőrizni, ezért kézenfekvő
megoldásnak tűnt a Chromium \texttt{-{}-lang} kapcsolójával felüldefiniálni a rendszer
beállítást, azonban az nem működött.

\begin{itemize}
    \item Mi hiányzott? -> bug report
    \item Hogyan lett megoldva, hogy a nyelvi beállítás minden processzre érvényes legyen?
    \item \texttt{ContentBrowserClient::AppendExtraCommandLineSwitches()}
    \item Qt API-n keresztül beállított locale is érvényes lesz minden processzre
    \item \texttt{QLocale::setDefault(QLocale("en"))}
    \item \texttt{ui::ResourceBundle} -> Mi ez? Miért kell neki locale?
    \item Qt 5.5
\end{itemize}
\pagebreak
