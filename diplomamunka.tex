...
\subsection{Process}
A QtWebEngine Process a Chromium Render Process-t fedi el. Ez processz felel a web tartalom
rendereléséért és a \texttt{JavaScript} kódok végrehajtásáért. A QtWebEngine \\
Process-t a Browser Process fogja elindítani, OS X-en és Linuxon sandbox-ban.

\subsection{Core}
\begin{itemize}
    \item Ez tartalmazza a Chromium fork-ot
    \item internal API: elfedi Content API-t a publikus API-k elől
    \item Design Pattern
        \begin{itemize}
            \item Delegate osztályok
            \item Adapterek
        \end{itemize}
    \item QtWebChannel: browser és render process, a browser process-ben van a
        webengine API, DOM a render processzben, változtatni szeretnénk a DOM-on
\end{itemize}

\subsection{API}
\begin{itemize}
    \item design pattern: d-pointer vagy opaque pointer
        \begin{itemize}
            \item implementáció rejtve marad
            \item implementációban a változtatások nem sértik a bináris kompatibilitást
            \item private osztályok általában a core internal API-t hívják, vagy
                előfordul, hogy maga az osztály a core-ban a public API private osztálya
        \end{itemize}
    \item Bináris kompatibilitás: a Qt alkalmazás alatt újra buildelés nélkül ki lehet
        cserélni a Qt library-t egy újabb verzióra
    \item Ne változtassuk meg a public osztály méretét és elrendezését. Megoldás: d-pointer
    \item A 2 API (Quick és Widget) között vannak funkcióbeli különbségek
        (``az egyik tudja, a másik nem'')
    \item nem használhatják közvetlenül a chromium API-t -> a fordítási egységnek ne legyen
        chromium függősége (core API-ra ez értelemszerűen nem igaz)
\end{itemize}

\subsubsection{Core}
\begin{itemize}
    \item modul: QtWebEngineCore
    \item Új (5.7): publikus API
    \item olyan API-k melyeken a Widget és a Quick API-k osztoznak
    \item webengine és webenginewidgets modulok automatikusan behúzzák
    \item \texttt{QtWebEngineCore::initialize()} -> OpenGL context inicializálása, \\
        hogy megosztott legyen processzek között
\end{itemize}

\subsubsection{Widget}
\begin{itemize}
    \item modul: QtWebEngineWidgets
    \item \texttt{C++} osztályok widget GUI létrehozására
    \item view -> page: egy view-hoz, csak egy page tartozik! (mi értelme?)
    \item profile -> shared settings, scripts, cookies
\end{itemize}

\subsubsection{Quick}
\begin{itemize}
    \item modul: QtWebEngine
    \item QML típusok, QML/Quick alkalmazások létrehozására
    \item ugyanaz, mint a widget, csak a page a view-ba van integrálva
\end{itemize}
