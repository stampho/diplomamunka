...
Ha egy Qt-s alkalmazás használ egy Qt modult, akkor azt annak a projekt file-jában meg kell
adni:
...
fejezetben lesz szó.} modul a Core-hoz hasonlóan implicit linkelődik viszont ez már
letiltható (pl. konzolos alkalmazás esetén). Erre a modulra a grafikus felhasználói felület
megvalósításához van szükség.

Ahogy a Qt-s alkalmazások, használnak egy Qt-s modult, úgy a Qt-s modulok is függhetnek
egymástól. Az alkalmazáséhoz hasonlóan, a module projekt file-jában is jelölni kell a
...
eszközkészletet kínál a fejlesztők számára. A Qt GUI modul az, amely alacsony szintű API-t
...
alkalmazásunkhoz, feltételezvén azt, hogy az rendelkezik grafikus felülettel. Ha ez nem így
lenne, a Qt GUI modul letiltható ha a következő sort adjuk GUI nélküli alkalmazás
projekt file-jához:
\begin{verbatim}
    QT -= gui
\end{verbatim}
...
Jelenleg két egymástól eltérő lehetőség elérhető: \textbf{Widget} és \textbf{Quick}.
\cite{bib:qt-doc-qt-gui, bib:qt-doc-user-interfaces}

\subsubsection{Widget}
A widget API-n keresztül ``hagyományos'' felhasználói felületeket lehet megvalósítani,
melyeknek megjelenítéséhez a Qt a futtató operációs rendszer stílusát adja (\textit{native
look'n'feel}). A widgetek jellemzően olyan statikus GUI elemek, amelyek általában desktopon
használt alkalmazások használnak. Megjeleníthetnek vagy tárolhatnak információt és/vagy
felhasználói eseményeket kezelhetnek.
Például: gomb (\texttt{QButton}), címke (\texttt{QLabel}), görgetősáv (\texttt{QScrollBar}),
stb. Az a widget, aminek nincs szülője, az az ablak.
...
file-ból a User Interface Compiler (uic) generál buildelhető C++ forrás állományt.
...
Language). Ez a nyelv a Qt Qml modulon keresztül válik elérhetővé.
A modul rendelkezik C++ és QML API-val is, hogy QML nyelv saját
...
Chromium függőségeik. A publikus API-k számára közös adapter interfészt
...
A két API között előfordulhatnak funkcionalitásban előfordulhatnak különbségek, például az
egyikben elérhető egy API függvény a másikban még nem.
