...
a Qt fejlesztői dönteni kényszerültek: folytassák-e a WebKit-re épülő QtWebKit
projektjüket és maradjanak az Apple szárnysegédjei, vagy beálljanak az új lehetőséget
és irányt mutató Google mögé azáltal, hogy egy Blink-et beágyazó web  böngésző platform fölé
építenek egy új API-t. A döntés megkönnyítése érdekében kezdték el fejleszteni -- eleinte csak
kísérleti jelleggel -- a Chromium-ra épülő QtWebEngine-t. Végül a QtWebEngine és a Google
mellett döntöttek.
...
Chromium-ról és a web technológiákról. Dolgozatomban az ez idő alatt gyűjtött
...
\subsection{Chrome}
A Google saját Chromium-ra épülő böngészőalkalmazása a Google Chrome.
...
Egy másik, szintén népszerű Chromium-ra épülő böngészőalkalmazás az Opera.
...
ebből is látszik, hogy a Blink az egyik legfontosabb komponense a Chromium-nak.
A Chromium a 28-es verzió előtt WebKit-et használt erre célra. A Google a WebKit projektet
forkolta és nevezte át Blink-re.
\cite{bib:wiki-blink}
...
motor. Tulajdonképpen a WebKit-ből a Blink-ben csak a \\
...
eltekintve a Blink még mindig nagyon hasonlít a WebKit-re (WebCore-ra). A legtöbb változó és
...
A Chrome Compositor nem része a Blink-nek (ahogy a Skia sem). Ennek az az oka,
hogy a Chromium a böngészőalkalmazás felhasználói felületének (GUI) összeállításához is
nyújt API-kat (pl. Aura). A Chromium a GUI megrajzolásához is a Chrome Compositor-t
használja.
...
string azonosítót rendel a textúrához és lehetővé teszi, hogy az más OpenGL context-ben
is elérhető legyen.
\cite{bib:chromium-gpu, bib:chromium-oopifs}
...
\begin{description}
    ...
    \item[Single Process]
        ...
        Nincs külön Render Process. A rendering a Browser Process-ben, \\
...
Erre vagy akkor van szükség amikor két site hivatkozik script-ből egymásra (függnek
egymástól), vagy amikor a futó Render Process-ek száma elér egy korlátot.
...
a Browser Process-ben futtatni. A legtöbb plugin azonban nem futhat sandbox környezetben,
...
így szükség van egy \textit{interface}-re is, amin keresztül a funkciók megvalósíthatóak és
testreszabhatóak a beágyazó alkalmazásban anélkül, hogy a Chromium forráskódját módosítani
kellene. Ez az interface a Chromium Content API.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.41]{content-shell}
    \caption{
...
\begin{description}
    ...
    \item[Blink]
        ...
        Blink-en keresztül érhető el és közös processzben futnak.
    \item[Content]
...
új Render Process-t a \texttt{-{}-lang locale} kapcsolóval fogja indítani, ahol a
...
visszavonja a nyelvi beállítást). Mivel új nyelvet beállítani a már futó Render Process-ekre
...
QtWebKit-ből átvett és továbbfejlesztett (Navigation History, Find Text, Authentication
